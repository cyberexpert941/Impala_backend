...
class Controller {
    ...
    async list(req, res, next) {
        try {
            ...
            if (!(typeof query?.search === 'undefined')) {
                ...
                search_or_conditions.push({ "subtitle": new RegExp(`.*${query?.search}.*`, 'i') });
                search_or_conditions.push({ "badge": new RegExp(`.*${query?.search}.*`, 'i') });
    ...
    async list_frontend(req, res, next) {
        try {
            let { query, params, headers } = req;
            let filterQuery = {};
            let sort = { "published_date": -1, "createdAt": -1 };
            let per_page = parseInt(query?.per_page || "10");
            let page_no = 1;
            if ((!(typeof query?.page_no === 'undefined')) && parseInt(query?.page_no) != "0") {
                page_no = parseInt(query?.page_no);
