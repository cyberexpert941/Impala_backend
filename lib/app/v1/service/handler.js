...
class Controller {
    ...
    async list(req, res, next) {
        try {
            ...
            if (!(typeof query?.category === 'undefined') && query.status !== 'all') {
                and_conditions.push({ "service_category": query?.category });
            }
            ...
            if (user_type !== "SUPER_ADMIN" && country) {
                ...
                country_or_conditions.push({ "service_features_and_offers.country_price.country": country });
                and_conditions.push({
    ...
    async country_wise_list(req, res, next) {
        try {
            ...
            if (query?.search) {
                ...
                search_or_conditions.push({ service_name: regex });
                search_or_conditions.push({ service_category: regex });
            ...
            if (query?.service_category) {
                and_conditions.push({
                    service_category: query.service_category?.trim()
                });
            }
            // Filter by tags
            if (query?.tags) {
                    ...
                    $or: [
                        {
                            ...
                            pay_as_you_go_price: {
                                $elemMatch: {
                                    ...
                                    country: country // <- ensure it matches the selected country
                                }
                            }
                        },
                        {
